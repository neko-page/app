// script.js - 最终完整修复版（支持 URL 参数解除横屏限制）
// ==================== 全局变量 ====================
let components = [];
...
function checkOrientation() {
    ...
    if (forcePortrait) {
        ...
        console.log('💡 提示：移除 URL 中的 ?mobile=true 可恢复横屏限制');
    ...
    if (window.innerWidth < 1000 || window.innerHeight > window.innerWidth) {
        console.log('📱 检测到竖屏，显示警告');
        if (warning) warning.style.display = 'flex';
        if (app) app.style.display = 'none';
    } else {
        console.log('💻 检测到横屏，正常显示');
        if (warning) warning.style.display = 'none';
        if (app) app.style.display = 'flex';
    }
}
...
function saveComponent() {
    ...
    if (currentComponent.type === 'condition') {
        data.trueText = cleanText(form.querySelector('[name="trueText"]') ? form.querySelector('[name="trueText"]').value : '');
        data.falseText = cleanText(form.querySelector('[name="falseText"]') ? form.querySelector('[name="falseText"]').value : '');
        data.selector = cleanText(form.querySelector('[name="selector"]') ? form.querySelector('[name="selector"]').value : '@p');
    }
    ...
    for (let i = 0; i < components.length; i++) {
        ...
        switch (comp.type) {
            ...
            case 'entityNBT':
                ...
                break;
            case 'condition':
                item = { translate: '%%2', with: { rawtext: [ { selector: cleanText(comp.data && comp.data.selector ? comp.data.selector : '@p') }, { text: cleanText(comp.data && comp.data.trueText ? comp.data.trueText : '') }, { text: cleanText(comp.data && comp.data.falseText ? comp.data.falseText : '') } ] } };
                break;
            default:
                item = { text: '' };
        }
...
function renderPreviewText(rawtextArray) {
    ...
    for (let i = 0; i < rawtextArray.length; i++) {
        ...
        if (item.text) {
            ...
            if (conditionMet && item.with.rawtext[1] && item.with.rawtext[1].text) {
                const result = parseFormatCodes(item.with.rawtext[1].text, formatState);
                text = result.html;
                formatState = result.state;
            } else if (!conditionMet && item.with.rawtext[2] && item.with.rawtext[2].text) {
